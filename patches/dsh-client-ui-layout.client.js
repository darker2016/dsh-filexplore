window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* @returns resolved widths; 0 means visually closed (never unmounted).
		*/
		function computeColumns(viewport, sidebar, right, details) {
			const s = sidebar === 0 ? 56 : clampWidth(sidebar, 264, 420);
			const r0 = right === 0 ? 0 : clampWidth(right, 320, 760);
			const d0 = details === 0 ? 0 : clampWidth(details, 300, 520);
			if (s + r0 + d0 + 640 <= viewport) return {
				sidebar: s,
				center: viewport - s - r0 - d0,
				right: r0,
				details: d0
			};
			const d1 = d0 === 0 ? 0 : Math.max(300, viewport - s - r0 - 640);
			if (s + r0 + d1 + 640 <= viewport) return {
				sidebar: s,
				center: 640,
				right: r0,
				details: d1
			};
			const r1 = r0 === 0 ? 0 : Math.max(320, viewport - s - 640);
			if (s + r1 + 640 <= viewport) return {
				sidebar: s,
				center: 640,
				right: r1,
				details: 0
		...
		*/
		...
		/** Plugin right panel column grid item; width 0 keeps the subtree mounted. */
		...
		*/
		...
		function AppFrame({ useStore, useSessions, actions, renderSlot }) {
			...
			const cols = computeColumns(viewport, sidebarCollapsed ? 0 : panels.sidebar === 0 ? 280 : panels.sidebar, panels.right, detailsSession === void 0 ? 0 : panels.details);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				style: { gridTemplateColumns: `${cols.sidebar}px minmax(0, 1fr) ${cols.right}px ${cols.details}px` },
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(CenterColumn, { children: renderSlot("conversation", {}) }), (0, react_jsx_runtime.jsx)(RightColumn, { children: renderSlot("layout.right", {}) }), (0, react_jsx_runtime.jsx)(DetailsColumn, { children: renderSlot("details", {}) })] }),
					(0, react_jsx_runtime.jsx)("div", {
					...
					cols.right > 0 && (0, react_jsx_runtime.jsx)(DragHandle, {
						side: "right",
						left: viewport - cols.right - cols.details,
		...
		*/
		...
		var LayoutController = class {
			...
			*/
			...
			/** Open the plugin right panel (no-op when already open). */
			openRight() {
				this.#require().openRight();
			}
			/** Close the plugin right panel. */
			closeRight() {
				this.#require().closeRight();
			}
			/** Set the plugin right panel width (clamped to the contract range). */
			setRight(px) {
				this.#require().setRight(px);
			}
			#require() {
